...
def _open_database_get_dataframe():
    ...
    one_months_ago = currentDate - rd.relativedelta(days=3)
    two_weeks_ago = currentDate - rd.relativedelta(weeks=2)
    current_date_plus_one = currentDate + rd.relativedelta(days=1)

    acc_snapshot_agg_pipeline = [
        {
            "$match": {
                #"$and": [{"date": {"$gt": one_months_ago}}, {"date": {"$lt": two_weeks_ago}}]
                "$and": [{"date": {"$gt": one_months_ago}}]
            }
        },
    ]
    ...
    session_metrics_pipeline = [
        {
            "$match": {
                "$and": [{"session_start_time": {"$gt": one_months_ago}}]
            }
        }
    ]
    ...
    df.to_csv("3DaysData.csv", index=False, encoding="utf-8")
